...
class SearchLocationMapPage extends StatefulWidget {
  final String text;
  final double lat;
  final double long;
  const SearchLocationMapPage(
      {Key? key, required this.text, required this.lat, required this.long})
      : super(key: key);

  @override
  State<SearchLocationMapPage> createState() => _SearchLocationMapPageState();
}

class _SearchLocationMapPageState extends State<SearchLocationMapPage> {
  ...
  void _onMapCreated(GoogleMapController _cntlr) async {
    ...
    _long = widget.long.toString();
    _lat = widget.lat.toString();
  ...
  _handleTap(LatLng tappedPoint) async {
    ...
    _markers = [];
    setState(() {
      _textController.text = "${_placemark[0].name.toString()} ${_city}";
    });
  }
